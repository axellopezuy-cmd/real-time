...
mod tests {
    ...
    fn test_parse_nested() {
        let result = parse_html("<div><span></span></div>");
        assert_eq!(result.len(), 1);
    ...
    fn test_parse_void_element() {
        let result = parse_html("<div><br><img></div>");
        assert_eq!(result.len(), 1);
        assert_eq!(result[0].children.len(), 2);
        assert_eq!(result[0].children[0].tag, "br");
        assert_eq!(result[0].children[1].tag, "img");
    }

    #[test]
    fn test_parse_attributes() {
        let result = parse_html(r#"<div class="main" id="app"></div>"#);
        assert_eq!(result.len(), 1);
        assert!(result[0].attributes.contains(&("class".to_string(), "main".to_string())));
        assert!(result[0].attributes.contains(&("id".to_string(), "app".to_string())));
    }

    #[test]
    fn test_tolerant_invalid_html() {
        // Should not panic, produces partial result
    ...
    fn test_pretty_print_nested() {
        ...
        text_content: None, }];
        let html = pretty_print(&nodes);
        assert!(html.contains("<div class=\"main\">"));
